...
/**
 * The class that is responsible for when a standard for loop using an
 * index method is selected to be used in the AutoComplete gui
 */
public class IndexMethod implements AutoCompleter {
    ...
    /**
     * Constructor for the IndexMethod class that takes a unspecified list
     * as an argument
     * @param type
     */
    public IndexMethod(List type) {
        specifiedList = type;
    }
     ...
     * @throws IllegalArgumentException when a grid file is uploaded
     */
    ...
    public List allThatBeginWith(String prefix) {
        ...
        if (specifiedList != null) {
            ...
            for (int i = 0; i < specifiedList.size(); i++) {
                if (specifiedList.get(i).length() >= prefixLength) {
                    if ((specifiedList.get(i)).substring(0,
                            prefixLength).equalsIgnoreCase(prefix)) {
                        matches.add(specifiedList.get(i));
                    }
                }
            }
        } else {
    ...
    public boolean contains(String target) {
        ...
        if (file != null) {
            for (int i = 0; i < specifiedList.size(); i++){
                if (target.equalsIgnoreCase(specifiedList.get(i))){
                    isThereAMatch = true;
                }
            }
